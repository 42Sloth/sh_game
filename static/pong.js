...
var Game = {
  ...
  update: function () {
    if (!this.over) {
      ...
      else if (this.player_left.move === DIRECTION.DOWN) this.player_left.y += this.player_left.speed;
      // Move player if they player.move value was updated by a keyboard event
      if (this.player_right.move === DIRECTION.UP) this.player_right.y -= this.player_right.speed;
      else if (this.player_right.move === DIRECTION.DOWN) this.player_right.y += this.player_right.speed;
  ...
  listen: function () {
    document.addEventListener('keydown', function (key) {
      ...
      if (key.keyCode === 87)
        Pong.player_left.move = DIRECTION.UP;
      if (key.keyCode === 83)
        Pong.player_left.move = DIRECTION.DOWN
      if (key.keyCode === 38)
        Pong.player_right.move = DIRECTION.UP;
      if (key.keyCode === 40)
        Pong.player_right.move = DIRECTION.DOWN;
    });

    // Stop the player from moving when there are no keys being pressed.
    document.addEventListener('keyup', function (key) {
      if (key.keyCode == 87 || key.keyCode == 83)
        Pong.player_left.move = DIRECTION.IDLE;
      if (key.keyCode == 38 || key.keyCode == 40)
        Pong.player_right.move = DIRECTION.IDLE;
    });
  },
